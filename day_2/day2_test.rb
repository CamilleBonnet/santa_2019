require './day2'

opcode_1 = %w(1 9 10 3 2 3 11 0 99 30 40 50).map(&:to_i)
result_1 = %w(3500 9 10 70 2 3 11 0 99 30 40 50).map(&:to_i)

opcode_2 = %w(1 0 0 0 99).map(&:to_i)
result_2 = %w(2 0 0 0 99).map(&:to_i)

opcode_3 = %w(2 3 0 3 99).map(&:to_i)
result_3 = %w(2 3 0 6 99).map(&:to_i)

opcode_4 = %w(2 4 4 5 99 0).map(&:to_i)
result_4 = %w(2 4 4 5 99 9801).map(&:to_i)

opcode_5 = %w(1 1 1 4 99 5 6 0 99).map(&:to_i)
result_5 = %w(30 1 1 4 2 5 6 0 99).map(&:to_i)

# final_opc  = %w(1,12,2,3,1,1,2,3,1,3,4,3,1,5,0,3,2,13,1,19,1,19,10,23,1,23,13,27,1,6,27,31,1,9,31,35,2,10,35,39,1,39,6,43,1,6,43,47,2,13,47,51,1,51,6,55,2,6,55,59,2,59,6,63,2,63,13,67,1,5,67,71,2,9,71,75,1,5,75,79,1,5,79,83,1,83,6,87,1,87,6,91,1,91,5,95,2,10,95,99,1,5,99,103,1,10,103,107,1,107,9,111,2,111,10,115,1,115,9,119,1,13,119,123,1,123,9,127,1,5,127,131,2,13,131,135,1,9,135,139,1,2,139,143,1,13,143,0,99,2,0,14,0)
opcode_part_1 = %w(1 12 2 3 1 1 2 3 1 3 4 3 1 5 0 3 2 13 1 19 1 19 10 23 1 23 13 27 1 6 27 31 1 9 31 35 2 10 35 39 1 39 6 43 1 6 43 47 2 13 47 51 1 51 6 55 2 6 55 59 2 59 6 63 2 63 13 67 1 5 67 71 2 9 71 75 1 5 75 79 1 5 79 83 1 83 6 87 1 87 6 91 1 91 5 95 2 10 95 99 1 5 99 103 1 10 103 107 1 107 9 111 2 111 10 115 1 115 9 119 1 13 119 123 1 123 9 127 1 5 127 131 2 13 131 135 1 9 135 139 1 2 139 143 1 13 143 0 99 2 0 14 0).map(&:to_i)
opcode_part_3 = %w(1 12 2 3 1 1 2 3 1 3 4 3 1 5 0 3 2 13 1 19 1 19 10 23 1 23 13 27 1 6 27 31 1 9 31 35 2 10 35 39 1 39 6 43 1 6 43 47 2 13 47 51 1 51 6 55 2 6 55 59 2 59 6 63 2 63 13 67 1 5 67 71 2 9 71 75 1 5 75 79 1 5 79 83 1 83 6 87 1 87 6 91 1 91 5 95 2 10 95 99 1 5 99 103 1 10 103 107 1 107 9 111 2 111 10 115 1 115 9 119 1 13 119 123 1 123 9 127 1 5 127 131 2 13 131 135 1 9 135 139 1 2 139 143 1 13 143 0 99 2 0 14 0).map(&:to_i)
opcode_part_2 = %w(1 0 0 3 1 1 2 3 1 3 4 3 1 5 0 3 2 13 1 19 1 19 10 23 1 23 13 27 1 6 27 31 1 9 31 35 2 10 35 39 1 39 6 43 1 6 43 47 2 13 47 51 1 51 6 55 2 6 55 59 2 59 6 63 2 63 13 67 1 5 67 71 2 9 71 75 1 5 75 79 1 5 79 83 1 83 6 87 1 87 6 91 1 91 5 95 2 10 95 99 1 5 99 103 1 10 103 107 1 107 9 111 2 111 10 115 1 115 9 119 1 13 119 123 1 123 9 127 1 5 127 131 2 13 131 135 1 9 135 139 1 2 139 143 1 13 143 0 99 2 0 14 0).map(&:to_i)

r1 = Day2.process(opcode_1)
r2 = Day2.process(opcode_2)
r3 = Day2.process(opcode_3)
r4 = Day2.process(opcode_4)
r5 = Day2.process(opcode_5)

p "PART 1"
p "Tests"
p r1 == result_1
p r2 == result_2
p r3 == result_3
p r4 == result_4
p r5 == result_5

p "Results"
final_result = Day2.process(opcode_part_1)
other_final_result = Day2.other_process(opcode_part_1)
p 7210630 == final_result[0]
p 7210630 == other_final_result[0]
p final_result
p other_final_result
# p "----------"
# p "PART 2"
# p "Tests"
# p "Resutls"

# (0..99).each do |noun|
#   (0..99).each do |verb|
#     opcode = opcode_part_2.dup
#     opcode[1] = noun
#     opcode[2] = verb
#     result = Day2.process(opcode)
#     if result[0] == 19690720
#       p [noun, verb]
#       p 100 * noun + verb
#       break
#     end
#   end
# end
